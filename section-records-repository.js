...
(function(){
  ...
  window.editSectionRepoWorkspaceSafe=async function(id){id=String(id||'');const r=await get(id);if(!r)return;const cdw=await ensureDocumentWorkspace();const fileMeta={id:r.id,status:r.status,extension:r.extension,display_name:r.name,module_key:r.moduleKey,primary_record_type:'library_file',metadata:r.metadata||{documentUseMode:r.documentUseMode||'work'}};if(!cdw.isEditable(fileMeta)){alert(r.documentUseMode==='reference'?'هذا الملف مصنف كمرجع. غيّر نوعه من سجل الإصدارات أولًا.':'هذا النوع غير قابل للتحرير بمحرر Office.');return}cdw.edit(String(r.platformFileId||r.id),location.href);};
