...
const isLibraryFile=(f:any)=>/library/i.test(String(f?.module_key||''))||String(f?.module_key||'')==='manager_records_archive'||String(f?.primary_record_type||'')==='library_file'||String(f?.metadata?.source||'')==='manager_library';
...
Deno.serve(async(req)=>{
  ...
  function canManage(s:any,f:any){if(!s||!f||String(f.school_id)!==String(s.school_id))return false;if(f.ownership_scope==='user')return String(f.owner_user_id||'')===String(s.user_id||'')||isManager(s);return isManager(s)}
  async function delegatedTaskAccess(s:any,f:any,edit=false){if(!s||!f||String(f.school_id)!==String(s.school_id))return false;const {data:links}=await sb.from('platform_file_links').select('record_id').eq('school_id',s.school_id).eq('file_id',f.id).eq('module_key','central_tasks').eq('record_type','delegated_library_file').eq('relation_type','delegated_work_file').is('deleted_at',null);const tids=(links||[]).map((x:any)=>String(x.record_id||'')).filter(isUuid);if(!tids.length)return false;let q=sb.from('platform_tasks').select('id,status,assigned_to,assignee_email').eq('school_id',s.school_id).in('id',tids);if(edit)q=q.in('status',['active','in_progress','returned']);else q=q.not('status','in','(withdrawn,archived)');if(isUuid(String(s.user_id||'')))q=q.eq('assigned_to',s.user_id);else if(s.user_email)q=q.eq('assignee_email',String(s.user_email).trim().toLowerCase());else return false;const {data}=await q.limit(1);return !!(data&&data.length)}
  async function canRead(s:any,f:any){return canManage(s,f)||(await delegatedTaskAccess(s,f,false))}
  async function canEdit(s:any,f:any){return !!s&&!!f&&String(f.school_id)===String(s.school_id)&&f.status==='active'&&editableExt.has(extOf(f))&&isLibraryFile(f)&&modeOf(f)!=='reference'&&(canManage(s,f)||(await delegatedTaskAccess(s,f,true)))}
  ...
  async function moveLibraryLinks(oldFile:any,newFile:any,userId:string){
    ...
    for(const l of links||[]){
      const isLibrary=String(l.relation_type||'')==='library_file'||['library_file','library'].includes(String(l.record_type||''));const isDelegatedWork=String(l.module_key||'')==='central_tasks'&&String(l.record_type||'')==='delegated_library_file'&&String(l.relation_type||'')==='delegated_work_file';if(!isLibrary&&!isDelegatedWork)continue;
  ...
  try{
    ...
    if(action==='capabilities'){const f=await getFile(String(body.fileId||''),String(s.school_id));return json({editable:await canEdit(s,f),configured:!!(documentServerUrl&&jwtSecret),extension:extOf(f),libraryFile:!!f&&isLibraryFile(f),mode:modeOf(f)});}
    if(action==='open-session'){
      ...
      const f=await getFile(String(body.fileId||''),String(s.school_id));if(!f)return json({error:'الملف غير موجود'},404);if(!(await canEdit(s,f)))return json({error:modeOf(f)==='reference'?'تم تصنيف هذا الملف كمرجع للقراءة. غيّر نوع الاستخدام إلى ملف عمل أو سجل مستمر أولًا.':'لا توجد صلاحية لتحرير هذا الملف أو أن صيغته غير مدعومة.'},403);
    ...
    if(action==='versions'){
      const f=await getFile(String(body.fileId||''),String(s.school_id));if(!f)return json({error:'الملف غير موجود'},404);if(!(await canRead(s,f)))return json({error:'لا توجد صلاحية'},403);const rows=await versionChain(f,String(s.school_id));return json({currentFile:f,versions:rows.map(x=>({...x,storage_path:undefined,bucket_name:undefined}))});
    }
    if(action==='set-mode'){
