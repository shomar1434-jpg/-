(function(){
  ...
  async function getMemberships(){var a=await cloudMemberships();if(a.length)return a;var b=await directMemberships();return unique(b.concat(localMemberships()))}
  ...
  async function switchTo(m){
    ...
    try{
      ...
      var q='?role='+encodeURIComponent(m.role)+'&uid='+encodeURIComponent(m.userId||'')+'&schoolId='+encodeURIComponent(m.schoolId)+'&school_name='+encodeURIComponent(m.schoolName||'')+'&membershipId='+encodeURIComponent(m.membershipId||'')+'&schoolMode=independent&independent=true';
  ...
  function openModal(rows,force){ensureStyle();closeModal();rows=rows||cache;var sid=activeSchoolId(),ar=roleMeta(activeRole()).app,groups=group(rows);var modal=document.createElement('div');modal.id='multiSchoolModal';modal.innerHTML='<div class="msShade"></div><div class="msBox"><div class="msHead"><div><h2>المدرسة والدور الحالي</h2><p>يمكن للحساب الواحد العمل في أكثر من مدرسة أو دور. كل مدرسة تحتفظ ببياناتها وتقاريرها وملفاتها مستقلة تمامًا.</p></div><button class="msClose" type="button">×</button></div>'+(groups.length?'<div class="msGrid">'+groups.map(function(g){var cur=String(g.id)===String(sid);return '<section class="msCard '+(cur?'current':'')+'"><div class="msSchoolName">🏫 '+esc(g.name)+'</div><div class="msCode">'+esc(g.code||g.id)+'</div><div class="msRoles">'+g.roles.map(function(m){var c=cur&&m.app===ar;return '<button type="button" class="msRole '+(c?'current':'')+'" data-ms="'+esc(m.membershipId||m.schoolId+'|'+m.role)+'">'+esc(roleLabel(m.role,m.roleLabel))+(c?' — الحالي':'')+'</button>'}).join('')+'</div></section>'}).join('')+'</div>':'<div class="msEmpty">لم يتم العثور على عضويات مدارس لهذا الحساب. ستبقى المدرسة الحالية دون تغيير.</div>')+'</div>';document.body.appendChild(modal);modal.querySelector('.msClose').onclick=function(){if(!force)closeModal()};modal.querySelector('.msShade').onclick=function(){if(!force)closeModal()};Array.from(modal.querySelectorAll('.msRole')).forEach(function(btn){btn.onclick=function(){var id=btn.getAttribute('data-ms');var m=rows.find(function(x){return (x.membershipId||x.schoolId+'|'+x.role)===id});switchTo(m)}})}
