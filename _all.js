        ...
        window.fs = {
            doc: function(){ return null; },
            setDoc: async function(){ return null; },
            getDoc: async function(){ return null; },
            updateDoc: async function(){ return null; },
            collection: function(){ return null; },
            onSnapshot: function(){ return function(){}; },
            query: function(){ return null; },
            addDoc: async function(){ return null; },
            deleteDoc: async function(){ return null; }
        };
        ...
        function normalizeUserActivationState(u){
            if(!u || typeof u !== 'object') return u;
            const active = (u.status === 'active' || u.isActive === true || u.active === true);
            if(active){ u.status = 'active'; u.isActive = true; u.active = true; }
            else { u.status = u.status || 'pending'; u.isActive = false; u.active = false; }
            return u;
        }

        function collectLocalUsersForWorkspace(){
            const byKey = new Map();
            const add = (user) => {
                if(!user || typeof user !== 'object') return;
                const email = String(user.email || '').trim().toLowerCase();
                if(!email) return;
                const id = String(user.id || email);
                const key = id + '|' + email + '|' + String(user.schoolId || '');
                const next = normalizeUserActivationState({ ...user });
                const prev = byKey.get(key);
                const prevActive = prev && (prev.status === 'active' || prev.isActive === true || prev.active === true);
                const nextActive = next.status === 'active' || next.isActive === true || next.active === true;
                if(!prev || nextActive || !prevActive) byKey.set(key, next);
            };
            const read = (raw) => {
                try {
                    const parsed = JSON.parse(raw || 'null');
                    if(Array.isArray(parsed)) parsed.forEach(add);
                    else if(parsed && Array.isArray(parsed.users)) parsed.users.forEach(add);
                } catch(e){}
            };
            ['smart_school_users','offline_users_backup','schoolUsers'].forEach(k => read(localStorage.getItem(k)));
            for(let i=0;i<localStorage.length;i++){
                const k = localStorage.key(i);
                if(/user|school/i.test(k || '')) read(localStorage.getItem(k));
            }
            return Array.from(byKey.values());
        }

        function saveUnifiedUserStores(users){
            const cleaned = (Array.isArray(users) ? users : []).map(u => normalizeUserActivationState({ ...u }));
            localStorage.setItem('smart_school_users', JSON.stringify(cleaned));
            localStorage.setItem('offline_users_backup', JSON.stringify(cleaned));
            localStorage.setItem('schoolUsers', JSON.stringify(cleaned));
            if(typeof allUsers !== 'undefined') allUsers = cleaned;
            return cleaned;
        }
        ...
        const submitSchoolRequest = async () => {
            ...
            if (allUsers.some(u => String(u.email || '').toLowerCase() === emailKey)) { showToast('يوجد طلب أو حساب سابق بنفس البريد'); return; }
            const requestId = 'school_req_' + Date.now() + '_' + Math.random().toString(36).slice(2);
            const schoolId = makeSchoolId(schoolName, email);
            const managerData = {
                id: requestId,
                name: managerName,
                email,
                ...
                status: 'pending',
                isActive: false,
                source: 'school_request',
                workspaceReady: false,
                registrationToken: 'REG-' + Math.random().toString(36).slice(2, 8).toUpperCase(),
                dateJoined: new Date().toLocaleString('ar-SA')
            };
            const schools = JSON.parse(localStorage.getItem('smart_school_schools') || '[]');
            schools.push({ id: schoolId, schoolId, schoolName, managerEmail: email, managerName, status: 'pending', source: 'school_request', requestId, createdAt: new Date().toISOString() });
            ...
            try {
                if (window.db && window.fs) {
                    await window.fs.setDoc(window.fs.doc(window.db, 'artifacts', window.appId, 'public', 'data', 'users', requestId), managerData, { merge: true });
                    await window.fs.setDoc(window.fs.doc(window.db, 'artifacts', window.appId, 'public', 'data', 'schools', schoolId), schools[schools.length-1], { merge: true });
                }
            } catch(e) { console.warn('School request cloud sync skipped', e); }
            closeModal('school-request-modal');
            showToast('تم إرسال طلب المدرسة الجديدة. سيظهر لدى مدير النظام للاعتماد.');
            if (document.getElementById('user-management-modal') && document.getElementById('user-management-modal').style.display === 'flex') {
                allUsers.splice(0, allUsers.length, ...JSON.parse(localStorage.getItem('smart_school_users') || '[]'));
                renderUserTable();
            }
        };
        ...
        const schoolSlug = (value) => {
            const source = String(value || 'school').trim().toLowerCase();
            const latin = source.replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
            if (latin) return latin.slice(0, 28);
            return 'school-' + Math.random().toString(36).slice(2, 8);
        };

        const getSchoolsStore = () => {
            try { return JSON.parse(localStorage.getItem('smart_school_schools') || '[]'); } catch(e) { return []; }
        };

        const saveSchoolsStore = (schools) => {
            localStorage.setItem('smart_school_schools', JSON.stringify(Array.isArray(schools) ? schools : []));
        };

        const absoluteUrlFor = (file, params = {}) => {
            const base = new URL(file, window.location.href);
            Object.entries(params).forEach(([k,v]) => base.searchParams.set(k, v));
            return base.href;
        };

        const buildSchoolLinks = (school) => {
            const schoolId = school.schoolId || school.id;
            const registrationCode = school.registrationCode || ('REG-' + String(schoolId || '').slice(-6).toUpperCase());
            return {
                login: absoluteUrlFor('school-login.html', { schoolId }),
                register: absoluteUrlFor('register.html', { schoolId, register: registrationCode })
            };
        };

        const copyText = async (text, label='الرابط') => {
            try {
                await navigator.clipboard.writeText(text);
                showToast(`تم نسخ ${label}`);
            } catch(e) {
                prompt(`انسخ ${label}:`, text);
            }
        };

        const openSchoolManagement = () => {
            if (currentUser?.role !== 'leadership') return showToast('إدارة المدارس مخصصة لمدير النظام فقط');
            document.getElementById('school-management-modal').style.display = 'flex';
            renderSchoolsManagement();
        };

        const createSchoolFromAdminPanel = async () => {
            const schoolName = document.getElementById('admin-school-name').value.trim();
            const managerName = document.getElementById('admin-manager-name').value.trim();
            const email = document.getElementById('admin-manager-email').value.trim().toLowerCase();
            const password = document.getElementById('admin-manager-password').value.trim();
            if (!schoolName || !managerName || !email || !password) return showToast('يرجى تعبئة اسم المدرسة واسم المدير والبريد والرقم السري');
            const registrationCode = 'REG-' + Math.random().toString(36).slice(2, 8).toUpperCase();
            try {
                if (window.SmartSchoolSupabase) {
                    const base = new URL('.', location.href).href;
                    const result = await window.SmartSchoolSupabase.createSchoolWithManager({ schoolName, managerName, email, password, registrationCode, loginLink: base + 'school-login.html', registrationLink: base + 'register.html' });
                    const schools = getSchoolsStore().filter(s => String(s.schoolId || s.id) !== String(result.school.schoolId));
                    schools.push(result.school);
                    saveSchoolsStore(schools);
                    const users = (typeof collectLocalUsersForWorkspace === 'function') ? collectLocalUsersForWorkspace() : JSON.parse(localStorage.getItem('smart_school_users') || '[]');
                    users.push(result.manager);
                    (typeof saveUnifiedUserStores === 'function') ? saveUnifiedUserStores(users) : localStorage.setItem('smart_school_users', JSON.stringify(users));
                    allUsers = users;
                    ['admin-school-name','admin-manager-name','admin-manager-email','admin-manager-password'].forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });
                    renderSchoolsManagement();
                    showToast('تم إنشاء المدرسة وتفعيل مديرها على Supabase');
                    return;
                }
            } catch (err) {
                console.error('Supabase create school failed', err);
                showToast('خطأ Supabase: ' + (err.message || JSON.stringify(err)));
                return;
            }
            let schools = getSchoolsStore();
            let users = (typeof collectLocalUsersForWorkspace === 'function') ? collectLocalUsersForWorkspace() : JSON.parse(localStorage.getItem('smart_school_users') || '[]');
            if (users.some(u => String(u.email || '').toLowerCase() === email && u.isDeleted !== true)) return showToast('يوجد حساب بنفس البريد');
            const schoolId = 'school_' + schoolSlug(schoolName) + '_' + Date.now().toString(36).slice(-5);
            const managerId = 'manager_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6);
            const school = { id: schoolId, schoolId, schoolName, schoolCode: schoolId, managerId, managerEmail: email, managerName, status: 'active', registrationCode, createdAt: new Date().toISOString(), createdBy: currentUser?.id || 'root' };
            const manager = { id: managerId, name: managerName, email, password, role: 'leadership', status: 'active', isActive: true, accountType: 'school_manager', isPrimaryManager: true, schoolId, schoolName, registrationCode };
            schools.push(school); users.push(manager);
            saveSchoolsStore(schools);
            (typeof saveUnifiedUserStores === 'function') ? saveUnifiedUserStores(users) : localStorage.setItem('smart_school_users', JSON.stringify(users));
            allUsers = users;
            ['admin-school-name','admin-manager-name','admin-manager-email','admin-manager-password'].forEach(id => { const el=document.getElementById(id); if(el) el.value=''; });
            renderSchoolsManagement();
            showToast('تم إنشاء المدرسة وتفعيل مديرها محليًا');
        };

        const activateSchoolManager = async (schoolId) => {
            try {
                if (window.SmartSchoolSupabase) {
                    await window.SmartSchoolSupabase.updateSchoolStatus(schoolId, 'active');
                }
            } catch(e) { console.warn('Supabase activate school failed', e); }
            let schools = getSchoolsStore();
            let users = (typeof collectLocalUsersForWorkspace === 'function') ? collectLocalUsersForWorkspace() : JSON.parse(localStorage.getItem('smart_school_users') || '[]');
            const school = schools.find(s => String(s.schoolId || s.id) === String(schoolId));
            if (!school) return showToast('لم يتم العثور على المدرسة');
            const email = String(school.managerEmail || '').toLowerCase();
            let manager = users.find(u => String(u.email || '').toLowerCase() === email);
            if (!manager) { manager = { id: school.managerId || ('manager_' + Date.now()), name: school.managerName, email, password: school.password || '123456', role: 'leadership' }; users.push(manager); }
            Object.assign(manager, { ...manager, isActive: true, status: 'active', accountType: 'school_manager', isPrimaryManager: true, schoolId: school.schoolId || school.id, schoolName: school.schoolName, registrationCode: school.registrationCode || ('REG-' + Math.random().toString(36).slice(2, 8).toUpperCase()) });
            school.status = 'active'; school.managerId = manager.id;
            if (!school.registrationCode) school.registrationCode = manager.registrationCode || ('REG-' + Math.random().toString(36).slice(2, 8).toUpperCase());
            saveSchoolsStore(schools);
            (typeof saveUnifiedUserStores === 'function') ? saveUnifiedUserStores(users) : localStorage.setItem('smart_school_users', JSON.stringify(users));
            allUsers = users;
            renderSchoolsManagement();
            showToast('تم تفعيل مدير المدرسة وربطه بمدرسته');
        };

        const renderSchoolsManagement = async () => {
            const body = document.getElementById('schools-management-body');
            if (!body) return;
            let schools = getSchoolsStore();
            try {
                if (window.SmartSchoolSupabase) {
                    schools = await window.SmartSchoolSupabase.listSchools();
                    saveSchoolsStore(schools);
                }
            } catch(e) { console.warn('Supabase list schools failed', e); }
            const users = (typeof collectLocalUsersForWorkspace === 'function') ? collectLocalUsersForWorkspace() : JSON.parse(localStorage.getItem('smart_school_users') || '[]');
            const schoolRequests = users.filter(u => u && u.accountType === 'school_manager' && u.schoolId && u.isDeleted !== true);
            schoolRequests.forEach(u => {
                if (!schools.some(s => String(s.schoolId || s.id) === String(u.schoolId))) {
                    schools.push({ id: u.schoolId, schoolId: u.schoolId, schoolName: u.schoolName || 'مدرسة غير مسماة', managerEmail: u.email, managerName: u.name, managerId: u.id, status: u.isActive ? 'active' : 'pending', registrationCode: u.registrationCode || ('REG-' + Math.random().toString(36).slice(2,8).toUpperCase()), source: u.source || 'users' });
                }
            });
            saveSchoolsStore(schools);
            if (!schools.length) {
                body.innerHTML = '<tr><td colspan="4" class="p-6 text-center text-slate-400 text-xs">لا توجد مدارس مستقلة حتى الآن. أنشئ مدرسة من النموذج أعلاه أو اعتمد طلب مدرسة جديد.</td></tr>';
                return;
            }
            body.innerHTML = '';
            schools.slice().reverse().forEach(school => {
                ...
                body.innerHTML += `<tr class="hover:bg-slate-50">
                    <td class="p-3 border-b font-bold text-xs text-slate-700">${school.schoolName || 'مدرسة بدون اسم'}<div class="text-[10px] text-slate-400 mt-1">${schoolId}</div></td>
                    <td class="p-3 border-b text-xs text-slate-600 font-bold">${school.managerName || 'بدون اسم'}<div class="text-[10px] text-slate-400 mt-1">${school.managerEmail || ''}</div></td>
                    <td class="p-3 border-b text-center">${isActive ? '<span class="text-green-600 font-bold text-xs">مفعلة</span>' : '<span class="text-amber-600 font-bold text-xs">معلقة</span>'}</td>
                    <td class="p-3 border-b text-center">
                        <div class="flex flex-wrap justify-center gap-2">
                            ${!isActive ? `<button onclick="activateSchoolManager('${safeJs(schoolId)}')" class="bg-teal-700 text-white px-3 py-1 rounded-lg text-[10px] font-bold">تفعيل المدير</button>` : ''}
                            <button onclick="copyText('${safeJs(links.login)}','رابط دخول المدرسة')" class="bg-slate-700 text-white px-3 py-1 rounded-lg text-[10px] font-bold">نسخ دخول المدرسة</button>
                            <button onclick="copyText('${safeJs(links.register)}','رابط تسجيل المستخدمين')" class="bg-emerald-700 text-white px-3 py-1 rounded-lg text-[10px] font-bold">نسخ رابط التسجيل</button>
                            <a href="${links.login}" target="_blank" class="bg-blue-700 text-white px-3 py-1 rounded-lg text-[10px] font-bold">فتح البوابة</a>
                        </div>
                    </td>
                </tr>`;
            });
        ...
        const renderGrid = (role) => {
            ...
            if (role === 'leadership') {
                ...
                schoolCard.innerHTML = `<h2 class="font-bold text-xl text-slate-800">🏫 إدارة المدارس</h2><p class="text-[10px] text-teal-600 mt-2 font-bold">إنشاء المدارس وربط المديرين والروابط المستقلة</p><button class="mt-6 bg-emerald-700 text-white px-6 py-2 rounded-xl w-full font-bold shadow-md">فتح الإدارة</button>`;
                schoolCard.onclick = openSchoolManagement;
...
(function(){
 if(window.__safeSmartAiSuite) return; window.__safeSmartAiSuite=true;
 function $(id){return document.getElementById(id)}
 function safe(v){return String(v||'').replace(/[<>]/g,'')}
 function out(id,t){var e=$(id); if(e)e.textContent=t}
 var p=$('safeAiPanel'), b=$('safeAiLaunch'), c=$('safeAiClose');
 if(b) b.onclick=function(){p.classList.add('open')}; if(c) c.onclick=function(){p.classList.remove('open')}; if(p) p.onclick=function(e){if(e.target===p)p.classList.remove('open')};
 document.querySelectorAll('.spa-tab').forEach(function(btn){btn.onclick=function(){document.querySelectorAll('.spa-tab').forEach(x=>x.classList.remove('active'));btn.classList.add('active');document.querySelectorAll('.spa-sec').forEach(s=>s.classList.toggle('active',s.dataset.sec===btn.dataset.tab));}});
 var tabs=document.querySelectorAll('.spa-tab'); if(tabs[0]) tabs[0].classList.add('active');
 var analyze=$('safeAiAnalyze'); if(analyze) analyze.onclick=function(){var t=$('safeAiAdmin').value||''; out('safeAiAdminOut','ملخص ذكي: '+(t?('تم تحليل النص ويحتوي على '+t.trim().split(/\s+/).filter(Boolean).length+' كلمة تقريبًا.'): 'أدخل نصًا أولًا.')+'\n\nتوصيات:\n• تحديد مسؤول تنفيذ.\n• تحويل البنود إلى مهام.\n• حفظ المخرج في الأرشيف أو محفظة الاجتماعات.')};
 var write=$('safeAiWrite'); if(write) write.onclick=function(){var t=$('safeAiWriter').value||''; out('safeAiWriterOut', t?'بناءً على ما ورد، نوصي بما يلي:\n'+t.split(/\n+/).filter(Boolean).map(function(x,i){return (i+1)+'. '+x.trim()}).join('\n'):'أدخل نصًا أولًا.')};
 var att=$('safeAiAttendance'); if(att) att.onclick=function(){var t=$('safeAiMeet').value||''; var names=[...new Set(t.split(/[\n,،;؛]+/).map(x=>x.trim()).filter(x=>x.length>1 && !/دخل|خرج|انضم|غادر/.test(x)))]; out('safeAiMeetOut',names.length?'قائمة الحضور الفعلية:\n• '+names.join('\n• '):'لم يتم العثور على أسماء واضحة.')};
 var search=$('safeAiDoSearch'); if(search) search.onclick=function(){var q=$('safeAiSearch').value||''; var text=document.body.innerText||''; var ok=q && text.includes(q); out('safeAiSearchOut', ok?'تم العثور على نتائج في الصفحة الحالية.':'لا توجد نتائج واضحة في الصفحة الحالية.')};
 var save=$('safeAiSaveSchool'); if(save) save.onclick=function(){localStorage.setItem('smartSchool.currentSchool', JSON.stringify({schoolName:$('safeAiSchool').value||'مدرسة', schoolId:$('safeAiSchoolId').value||'SCH-DEMO'})); out('safeAiSyncOut','تم حفظ بيئة المدرسة محليًا.')};
 var sync=$('safeAiSync'); if(sync) sync.onclick=function(){localStorage.setItem('smartSchool.lastSync', new Date().toISOString()); out('safeAiSyncOut','تم تجهيز المزامنة المحلية. عند اكتمال ربط Supabase ستتم المزامنة السحابية الحقيقية.')};
})();
