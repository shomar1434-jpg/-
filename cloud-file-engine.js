(function(){
  ...
  async function open(fileId){const preview=window.open('about:blank','_blank');if(preview){try{preview.opener=null;preview.document.title='جارٍ فتح الشاهد...';preview.document.body.innerHTML='<div dir="rtl" style="font-family:Tahoma,Arial,sans-serif;padding:30px;text-align:center">جارٍ التحقق من الشاهد وفتح المعاينة...</div>'}catch(_){ }}try{const x=await signedUrl(fileId);if(!x?.signedUrl)throw new Error('لم يُرجع الخادم رابط معاينة صالحًا');if(preview&&!preview.closed){preview.location.replace(x.signedUrl)}else throw new Error('حظر المتصفح نافذة المعاينة. اسمح بالنوافذ المنبثقة لهذا الموقع ثم حاول مرة أخرى.');return x}catch(e){try{if(preview&&!preview.closed){preview.document.body.innerHTML='<div dir="rtl" style="font-family:Tahoma,Arial,sans-serif;padding:30px;text-align:center;color:#b91c1c">تعذر فتح الشاهد: '+String(e?.message||e).replace(/[&<>]/g,'')+'</div>'}}catch(_){ }throw e}}
