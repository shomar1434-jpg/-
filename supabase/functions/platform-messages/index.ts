...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==='sent'){
   const {data,error}=await sb.from('internal_messages').select('*,internal_message_recipients(recipient_user_id,recipient_name,recipient_role,read_at,acknowledged_at,action_status),internal_message_attachments(id,file_id,file_name,mime_type,file_size,source)').eq('school_id',schoolId).eq('sender_user_id',userId).order('created_at',{ascending:false}).limit(150);if(error)throw error;return json({messages:(data||[]).map((m:any)=>{const rr=m.internal_message_recipients||[];return {...m,read_count:rr.filter((x:any)=>x.read_at).length,ack_count:rr.filter((x:any)=>x.acknowledged_at).length,recipient_count:rr.length}})});
  }
  if(['archive-message','unarchive-message','pin','unpin','acknowledge','mark-unread','action-status'].includes(action)){
   const id=safe(body.messageId,60);if(!isUuid(id))return json({error:'معرف الرسالة غير صالح'},400);const {m,r}=await canAccess(id);if(!m||!r)return json({error:'الإجراء متاح للمستلم فقط'},403);const change:any={};if(action==='archive-message')change.archived_at=now;if(action==='unarchive-message')change.archived_at=null;if(action==='pin')change.pinned_at=now;if(action==='unpin')change.pinned_at=null;if(action==='acknowledge'){change.acknowledged_at=now;change.read_at=r.read_at||now}if(action==='mark-unread')change.read_at=null;if(action==='action-status'){const st=['accepted','in_progress','done','declined'].includes(String(body.status))?String(body.status):'none';change.action_status=st;change.action_note=safe(body.note,1000)||null;change.read_at=r.read_at||now;if(st==='done'&&m.require_ack&&!r.acknowledged_at)change.acknowledged_at=now;}const {error}=await sb.from('internal_message_recipients').update(change).eq('id',r.id);if(error)throw error;return json({ok:true,state:{...r,...change}});
  }
