...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==='health')return json({ok:true,version:'2.0.0',schoolId:s.school_id,userId:s.user_id,role:s.role});
  ...
  if(action==='create'){
   ...
   if(body.grant){
    const baseGrant={school_id:s.school_id,task_id:t.id,user_id:assignedTo,user_email:assigneeEmail,can_view:true,can_create:!!body.grant.canCreate,can_update:body.grant.canUpdate!==false,can_upload:body.grant.canUpload!==false,can_submit:body.grant.canSubmit!==false,can_approve:!!body.grant.canApprove,can_delete:!!body.grant.canDelete,starts_at:body.startDate||now,expires_at:body.dueDate?new Date(String(body.dueDate)+'T23:59:59Z').toISOString():null,granted_by:s.user_id};
    // عند تفويض مجموعة سجلات، ننشئ صلاحية دقيقة لكل سجل حتى يعمل الحارس على المسار الفعلي للسجل.
    const grantRows=delegatedRecords.length
      ? delegatedRecords.filter((r:any)=>r&&r.moduleKey&&r.recordType).map((r:any)=>({...baseGrant,module_key:safeKey(r.moduleKey),record_type:String(r.recordType),record_id:null,permission_scope:'record'}))
      : [{...baseGrant,module_key:safeKey(body.grant.moduleKey||row.module_key),record_type:body.grant.recordType||body.recordType||null,record_id:body.grant.recordId||body.recordId||null,permission_scope:body.grant.permissionScope==='record_group'?'module':(body.grant.permissionScope||'module')}];
  ...
  if(action==='reassign'){
   ...
   const reassignGrantBase={school_id:s.school_id,task_id:task.id,user_id:to,user_email:email,can_view:true,can_create:true,can_update:true,can_upload:true,can_submit:true,can_approve:false,can_delete:false,granted_by:s.user_id,expires_at:task.due_date?new Date(String(task.due_date)+'T23:59:59Z').toISOString():null};
   const reassignGrants=(reassignLinks||[]).length
     ? (reassignLinks||[]).map((r:any)=>({...reassignGrantBase,module_key:r.module_key,record_type:r.record_type,record_id:r.record_id||null,permission_scope:'record'}))
     : [{...reassignGrantBase,module_key:task.module_key,record_type:task.record_type,record_id:task.record_id,permission_scope:'module'}];
   await sb.from('task_access_grants').insert(reassignGrants);
  ...
  if(action==='transition'){
   ...
   const allowed:any={active:['in_progress','withdrawn'],transferred:['in_progress','withdrawn'],in_progress:['pending_approval','withdrawn'],returned:['in_progress','pending_approval'],pending_approval:['approved','returned','rejected'],approved:['archived','closed'],rejected:['archived'],withdrawn:['archived']};if(!(allowed[task.status]||[]).includes(target))return json({error:`انتقال الحالة غير مسموح: ${task.status} ← ${target}`},409);
   const changes:any={status:target};if(target==='approved')changes.progress_percent=100;if(['closed','archived'].includes(target))changes.closed_at=now;
   ...
   await event(task.id,target,body.note||'',task,data);const recipient=ownerOnly.has(target)?task.assigned_to:task.created_by;await notice(task.id,recipient,ownerOnly.has(target)?task.assignee_email:null,target,'تحديث على التكليف',`تم تحديث حالة التكليف "${task.title}" إلى ${target}`);return json({task:data});
  }
