...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const canonicalTaskRecords = async (task:any, existingLinks:any[] = []) => {
      ...
      const candidates = metadataRows.length ? metadataRows : (existingLinks || []).filter((r:any)=>r?.module_key && r?.record_type);
      if (!candidates.length && task?.module_key && task?.record_type) candidates.push({
        task_id: task.id,
    ...
    if (action === 'health') return json({ ok: true, service: 'platform-core', version:'2.2.0-canonical-delegation', requestId, schoolId: session.school_id, userId: session.user_id });
