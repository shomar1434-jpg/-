(function(){
 ...
 async function refresh(options={}){if(!window.CloudTaskEngine)return;try{const r=await CloudTaskEngine.list();const pending=window.__centralTaskPendingDeletes instanceof Set?window.__centralTaskPendingDeletes:new Set();let mapped=(r.tasks||[]).map(CloudTaskEngine.mapTask).filter(t=>!pending.has(String(t.task_id||t.id||'')));if(typeof mode==='function'&&mode()==='assignee'&&typeof isMine==='function'){
   // platform-tasks يعيد للمستخدم العادي قائمة مقيدة به وبمدرسته من الخادم؛
   // لا نعيد ترشيحها بقيم localStorage قديمة. المدير/الوكيل فقط يحتاجان
   // ترشيح "تكليفاتي" لأن صلاحية المالك تعيد كل تكليفات المدرسة.
   const serverScope=r?.scope||{};
   if(serverScope.isOwner===true || !serverScope.userId) mapped=mapped.filter(isMine);
