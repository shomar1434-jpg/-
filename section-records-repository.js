...
(function(){
  ...
  async function upload(e){
    ...
    for(const file of files){
      ...
      try{
        await put({name:file.name,type:file.type||'',kind:kind(file),size:file.size,file:file,folder:activeFolder||'عام',createdAt:new Date().toISOString(),role:ROLE});
        ok++;
  ...
  window.editSectionRepoSafe=async function(id){id=String(id||'');
  ...
  if(!window.__SECTION_REPO_ACTION_DELEGATION_V2__){
    ...
    document.addEventListener('click',async function(ev){
      ...
      try{
        ...
        if(action==='edit')return await window.editSectionRepoSafe(id);
