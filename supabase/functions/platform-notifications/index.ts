...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==='register-subscription'){
    const sub=body.subscription||{},endpoint=text(sub.endpoint,3000),p256dh=text(sub.keys?.p256dh,1000),auth=text(sub.keys?.auth,1000);if(!endpoint||!p256dh||!auth)return json({error:'SUBSCRIPTION_INVALID'},400);
  ...
  if(action==='record-event'){
    ...
    let recipient=text(body.recipientUserId||body.data?.recipient_user_id||body.data?.responsible_user_id||body.data?.reviewer_id||body.data?.requester_id||body.data?.supervisor_id,100);
    ...
    // كل حدث تشغيلي له مستقبل واحد فقط: المسؤول الفعلي المحفوظ في العلاقة.
