...
(function(){
  ...
  window.deleteSectionRepoFolderSafe=async function(){const name=String(activeFolder||'عام');if(name==='عام'){alert('لا يمكن حذف المجلد العام.');return;}const rows=await all();const count=rows.filter(r=>String(r.folder||'عام')===name).length;if(count){alert('لا يمكن حذف المجلد لأنه يحتوي على '+count+' ملف/ملفات. انقل الملفات أو احذفها أولًا حفاظًا على البيانات.');return;}if(!confirm('حذف المجلد «'+name+'»؟ لن يتم حذف أي ملف.'))return;saveFolders(folders().filter(f=>f!==name));setActiveFolder('عام');};
  ...
  window.editSectionRepoMetaSafe=async function(id){id=String(id||'');
  ...
  window.deleteSectionRepoSafe=async function(id){id=String(id||'');if(confirm('هل تريد حذف الملف؟')){await del(id);render();}};
