(function(){
 ...
 async function refresh(options={}){if(!window.CloudTaskEngine)return;try{const openTaskId=String(window.inlineWs?.task?.id||window.inlineWs?.task?.task_id||inlineWs?.task?.id||inlineWs?.task?.task_id||'');const workspaceWasOpen=!!openTaskId&&!document.getElementById('inlineWorkspace')?.classList.contains('hidden');const r=await CloudTaskEngine.list();const pending=window.__centralTaskPendingDeletes instanceof Set?window.__centralTaskPendingDeletes:new Set();const mapped=(r.tasks||[]).map(CloudTaskEngine.mapTask).filter(t=>!pending.has(String(t.task_id||t.id||'')));if(options.preserveOnEmpty&&mapped.length===0&&tasks().length>0){console.warn('أعاد الخادم قائمة فارغة بعد الحفظ؛ تم الاحتفاظ بالعرض الحالي مؤقتًا.');return;}try{saveTasks(mapped);}catch(cacheError){if(cacheError&&(cacheError.name==='QuotaExceededError'||cacheError.code===22)){console.warn('[central-task-cache-quota] تعذر تحديث الكاش المحلي، وسيستمر العرض من بيانات السحابة.',cacheError);toast('تم تحميل التكليفات سحابيًا، ويجري إصلاح مساحة التخزين المحلية.',false);}else throw cacheError;}renderAll();if(workspaceWasOpen){const fresh=mapped.find(t=>String(t.task_id||t.id||'')===openTaskId);if(fresh&&inlineWs?.task){inlineWs.task={...inlineWs.task,...(fresh._raw||{}),id:fresh.task_id||fresh.id};document.getElementById('inlineWorkspace')?.classList.remove('hidden');if(inlineWs.readiness)renderReadinessWorkspace();else renderInlineWorkspace();}}window.dispatchEvent(new CustomEvent('central-tasks:cloud-loaded',{detail:{count:mapped.length,workspacePreserved:workspaceWasOpen}}));}catch(e){console.warn('تعذر تحميل التكليفات السحابية، تم الإبقاء على النسخة المحلية المؤقتة:',e);toast(e.message||'تعذر تحميل التكليفات السحابية',false)}}
