...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const canReadDelegatedTaskFile=async(row:any)=>{if(!row||String(row.school_id)!==String(s.school_id))return false;const {data:links}=await sb.from('platform_file_links').select('record_id').eq('school_id',s.school_id).eq('file_id',row.id).eq('module_key','central_tasks').eq('record_type','delegated_library_file').eq('relation_type','delegated_work_file').is('deleted_at',null);const tids=(links||[]).map((x:any)=>x.record_id).filter(isUuid);if(!tids.length)return false;let q=sb.from('platform_tasks').select('id,status,assigned_to,assignee_email').eq('school_id',s.school_id).in('id',tids).not('status','in','(withdrawn,archived)');if(isUuid(s.user_id))q=q.eq('assigned_to',s.user_id);else if(s.user_email)q=q.eq('assignee_email',String(s.user_email).trim().toLowerCase());else return false;const {data:r}=await q.limit(1);return !!(r&&r.length)};
  const hasOpenDelegatedWork=async(row:any)=>{if(!row)return false;const {data:links}=await sb.from('platform_file_links').select('record_id').eq('school_id',s.school_id).eq('file_id',row.id).eq('module_key','central_tasks').eq('record_type','delegated_library_file').eq('relation_type','delegated_work_file').is('deleted_at',null);const tids=(links||[]).map((x:any)=>x.record_id).filter(isUuid);if(!tids.length)return false;const {data}=await sb.from('platform_tasks').select('id').eq('school_id',s.school_id).in('id',tids).in('status',['active','in_progress','returned','pending_approval']).limit(1);return !!(data&&data.length)};
  const canReadFile=async(row:any)=>{if(canRead(row))return true;if(!row||row.school_id!==s.school_id)return false;if(await canReadDelegatedTaskFile(row))return true;const {data:links}=await sb.from('platform_file_links').select('record_id').eq('school_id',s.school_id).eq('file_id',row.id).eq('module_key','internal_messages').eq('record_type','internal_message').eq('relation_type','attachment');const mids=(links||[]).map((x:any)=>x.record_id).filter(isUuid);if(!mids.length)return false;let q=sb.from('internal_message_recipients').select('id').eq('school_id',s.school_id).in('message_id',mids);if(isUuid(s.user_id))q=q.eq('recipient_user_id',s.user_id);else if(s.user_email)q=q.eq('recipient_email',String(s.user_email).trim().toLowerCase());else return false;const {data:r}=await q.limit(1);return !!(r&&r.length)};
  ...
  if(action==='list-by-link'){
   ...
   const {data,error}=await q;if(error)throw error;const rows:any[]=[];for(const x of (data||[])){if(x.platform_files?.status==='active'&&(await canReadFile(x.platform_files)))rows.push(x)}return json({links:rows});
  }
  ...
  if(['signed-url','trash','restore','purge','rename-file','move-file','usage'].includes(action)){
   const file=await getFile(body.fileId);if(!file)return json({error:'الملف غير موجود'},404);if(action==='signed-url'){if(!(await canReadFile(file)))return json({error:'لا توجد صلاحية'},403);if(!['active','archived'].includes(String(file.status||'')))return json({error:'الملف غير متاح للمعاينة'},409);const expires=Math.max(60,Math.min(Number(body.expiresIn)||300,maxSigned));const {data,error}=await sb.storage.from(file.bucket_name).createSignedUrl(file.storage_path,expires);if(error)throw error;await event('viewed',file);return json({signedUrl:data.signedUrl,expiresIn:expires})}
   if(!canManage(file))return json({error:'لا توجد صلاحية'},403);
   if(action==='trash'){if(await hasOpenDelegatedWork(file))return json({error:'لا يمكن حذف الملف لأنه مرتبط بتكليف قائم. أغلق أو اسحب التكليف أولًا ثم أعد المحاولة.',code:'FILE_HAS_ACTIVE_DELEGATION'},409);await sb.from('platform_files').update({status:'trashed',deleted_at:now}).eq('id',file.id);if(file.module_key==='school_readiness')await sb.from('school_readiness_evidence').update({status:'trashed',deleted_at:now}).eq('platform_file_id',file.id);await event('trashed',file);return json({ok:true})}
